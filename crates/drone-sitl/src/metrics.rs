use crate::scenario::{MetricKind, Target};
use drone_sim::runner::SimFrame;

pub fn compute(metric: &MetricKind, frames: &[SimFrame], target: &Target) -> f64 {
    match metric {
...
fn position_rms_z(frames: &[SimFrame], target: &Target) -> f64 {
    if frames.is_empty() {
        return 0.0;
    }
    let sum_sq: f64 = frames
        .iter()
        .map(|f| (f.state.position.z - target.altitude_z).powi(2))
        .sum();
    (sum_sq / frames.len() as f64).sqrt()
}

/// Maximum Z position error during whole flight
fn position_max_error_z(frames: &[SimFrame], target: &Target) -> f64 {
    frames
        .iter()
        .map(|f| (f.state.position.z - target.altitude_z).abs())
        .fold(0.0_f64, f64::max)
}

/// overshoot - how much percent drone overreached target
fn overshoot_percent(frames: &[SimFrame], target: &Target) -> f64 {
    let initial_z = frames.first().map(|f| f.state.position.z).unwrap_or(0.0);

    if initial_z >= target.altitude_z {
        return 0.0;
    }
    ...
    if max_z <= target.altitude_z {
        return 0.0;
    }

    let overshoot = max_z - target.altitude_z;
    let total_range = target.altitude_z - initial_z;
    (overshoot / total_range) * 100.0
}

/// settling time - how many seconds until error z < 0.1m stable
fn settling_time_s(frames: &[SimFrame], target: &Target) -> f64 {
    let threshold = 0.1;
    let last_violation = frames
        .iter()
        .enumerate()
        .rev()
        .find(|(_, f)| (f.state.position.z - target.altitude_z).abs() >= threshold);
...
mod tests {
    ...
    #[test]
    fn rms_dla_stalego_bledu() {
        // Zawsze 1m od celu → RMS = 1.0
        let frames: Vec<_> = (0..100).map(|i| frame(i as f64 * 0.01, 4.0)).collect();
        let rms = position_rms_z(&frames, &Target { altitude_z: 5.0 });
        assert!((rms - 1.0).abs() < 1e-10);
    }

    #[test]
    fn brak_overshootu_gdy_nie_osiagnieto_celu() {
        let frames: Vec<_> = (0..100)
            .map(|i| frame(i as f64 * 0.01, i as f64 * 0.04))
            .collect();
        // Max z = 3.96m, cel = 5.0m → brak overshootu
        let overshoot = overshoot_percent(&frames, &Target { altitude_z: 5.0 });
        assert_eq!(overshoot, 0.0);
    }

    #[test]
    fn settling_time_gdy_zawsze_w_progu() {
        let frames: Vec<_> = (0..100).map(|i| frame(i as f64 * 0.01, 5.0)).collect();
        assert_eq!(settling_time_s(&frames, &Target { altitude_z: 5.0 }), 0.0);
    }

    #[test]
    fn settling_time_gdy_na_koncu_w_progu() {
        ...
        let st = settling_time_s(&frames, &Target { altitude_z: 5.0 });
